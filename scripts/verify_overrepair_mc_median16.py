...
def main() -> None:
    # Load config and device (align arg names and error style)
    args = load_config()
    ...
    # Single clean CSV (aligned by itself)
    ...
    # Over-repair metrics (observed-only) against the original clean series
    ...
    sigma09 = compute_sigma09_per_feature(Xc, M)
    ...
    nrmse_rep = nrmse_timestep(Xc, y_pred, M, sigma09)
    ...
    nrmse_rep_global = nrmse_global(Xc, y_pred, M, sigma09)
    ...
    # Global NRMSE using per-feature sigma from this clean series
    ...
    mse_per_t = mse_rep
    valid = ~np.isnan(mse_per_t)
    idx = np.where(valid)[0]
    if idx.size == 0:
        print('- Per-timestamp MSE: no valid entries (all-missing).')
    else:
        order_desc = np.argsort(mse_per_t[idx])[::-1]
        order_asc = np.argsort(mse_per_t[idx])
        ...
        for r, t in enumerate(top_idx, 1):
            print(f"  {r:>4d}  {ts_s[int(t)]:>19s}  {fmt(mse_per_t[t])}")
