...
class SSMPrior(nn.Module):
    ...
    The prior always uses a low-rank correction on top of diagonal noise (rank >= 1).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer('sparsity_mask', self._build_band_mask(self.D, bandwidth=min(4, self.D - 1)))

        raw_A = torch.zeros(self.D, self.D, device=dev, dtype=dt)
        a = float(a_init)
        raw_diag = 0.5 * math.log((1.0 + a) / (1.0 - a)) if abs(a) < 0.999 else 0.0
        raw_A.fill_(0.0)
    ...
    def _build_band_mask(dim: int, bandwidth: int) -> torch.Tensor:
        ar = torch.arange(int(dim))
        mask = (ar.unsqueeze(1) - ar.unsqueeze(0)).abs() <= int(bandwidth)
    ...
    def predict(
        ...
        *,
        ...
        if m_prev.ndim != 2 or m_prev.shape[1] != self.D:
            raise ValueError('m_prev must have shape [B,D]')

        B = int(m_prev.shape[0])
        if P_prev.shape != (B, self.D, self.D):
            raise ValueError('P_prev must have shape [B,D,D] matching m_prev and latent_dim')

        dev = m_prev.device
        dt = m_prev.dtype

        if A is None or Q is None:
            A, Q = self.transition_matrices(device=dev, dtype=dt)[:2]
        else:
    ...
    def kl_q_prior(
        ...
        if mu_q.ndim != 2 or mu_q.shape[1] != self.D:
            raise ValueError('mu_q must have shape [B,D]')

        B = int(mu_q.shape[0])
        if chol_q.shape != (B, self.D, self.D):
        ...
        if P_pred.shape != (B, self.D, self.D):
